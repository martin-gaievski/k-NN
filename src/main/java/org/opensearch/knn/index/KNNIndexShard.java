...
public class KNNIndexShard {
    ...
    public void warmup() throws IOException {
        logger.info(String.format("[KNN] Warming up index: %s", getIndexName()));
        try (Engine.Searcher searcher = indexShard.acquireSearcher("knn-warmup")) {
            getAllEnginePaths(searcher.getIndexReader()).forEach((key, value) -> {
                try {
                    // FSDirectory fsDirectory = (FSDirectory) indexShard.store().directory();
        ...
        logger.info(String.format("[KNN] Directory has %d files", dir.listAll().length));
        AtomicBoolean needToWarmup = new AtomicBoolean(false);
        ...
        if (needToWarmup.get()) {
            try {
                ...
                int dim = 128, k = 100, numOfQueries = 500;
                Random rand = new Random();
                for (int n = 0; n < numOfQueries; n++) {
                    float[] qVector = new float[dim];
                    IntStream.range(0, dim).forEach(i -> qVector[i] = 180 * rand.nextFloat());
                    Query warmUpQuery = new KnnVectorQuery("target_field", qVector, k);
                    TopDocs topDocs = searcher.search(warmUpQuery, k);
                ...
                logger.info(String.format("[KNN] Warmup, tried to open file"), e);
            }
            logger.info(String.format("[KNN] Pre-loaded file"));
        }
    }
